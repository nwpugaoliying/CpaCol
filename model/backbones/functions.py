...
def visualization(feat, im_, pth='attn_maps_1/'):
    global im_count
    if im_count >= 50:
        im_count = 0
    background = np.zeros((256,128*2,3))
    N, H, W = feat.shape
    ...
    for i in range(4):
        ...
        background[:, 128:, :] = result
        cv2.imwrite(pth+str(im_count%50)+'.jpg', background)
        im_count += 1
    return 
